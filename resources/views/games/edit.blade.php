...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header text-center"><u>Edit {{ $game['name'] }}</u></div>
            ...
            @endif
                <div class="card-body text-center col d-flex justify-content-center ">
                    <form action="/games/update/{{ $game['id'] }}" method="post" class="w-75" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="id" value="{{ $game['id'] }}">
                        <div class="form-group">
                            <label for="gameName">Game name:</label>
                            <input type="text" value="{{$game['name']}}" class="form-control text-center" name="gameName">
                        </div>
                        {{-- CATEGORY --}}
                        <div id="Category" class="mt-3">
                            <label for="category">Select Category</label>
                            <select class="categorySelect2 from-control" name="category">
                                @foreach($categories as $category)
                                    @if(($game->category_id) && ($game->category_id == $category->id))
                        ...
                        <div id="Tags" class="mt-3">
                            ...
                            <select class="tagsSelect2 from-control w-75" name="tags[]"  multiple="multiple">
                                @foreach($tags as $tag)
                                    @if(($selectedTagsIds))
                                        @if(in_array($tag->id, array_values($selectedTagsIds)) )
                        ...
                        <div id="Companies" class="mt-3 d-flex flex-row-reverse align-items-center">
                            ...
                            <select class="categorySelect2 from-control w-75" name="companies[]"  multiple="multiple">
                                @foreach($companies as $company)
                                    @if(($selectedTagsIds))
                                        @if(in_array($company->id, array_values($selectedCompaniesIds)) )
                        ...
                        <div id="ReleashDate" class="mt-3">
                            ...
                            <input type="date" id="release_date" name="release_date" value="{{ date('Y-m-d', $game->release_date) }}">
                            <div class="col-lg-6 mx-auto p-2">
                        </div iv>
                        <div class="checkbox mb-2">
                        @if($game['is2D'])
                            <label><input type="checkbox" checked name="is2D"> Game is 2D</label>
                        @else
                            <label><input type="checkbox" name="is2D"> Game is 2D</label>
                        @endif
                        </div>
                        <h1 class="d-inline-flex text-center">Upload new images</h1>
                        <div id="image1" class="border rounded">
                            <!-- Upload image 1 input-->
                            <div class="input-group mb-3 px-2 py-2 rounded-pill bg-white shadow-sm">
                                <input id="upload1" type="file" name="gameImage1" onchange="readURL1(this);" class="form-control border-0">
